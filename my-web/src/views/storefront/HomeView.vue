<template>
  <div class="space-y-16 pb-20 w-full pt-0">
    
    <!-- Full Screen Width Hero Section -->
    <section class="relative overflow-hidden bg-neutral-950 text-white w-full border-b border-neutral-800 shadow-2xl min-h-[520px] flex items-center">
      <div class="absolute inset-0 bg-gradient-to-r from-neutral-950 via-neutral-950/85 to-transparent z-10"></div>
      <img
      ...
      <div class="relative z-20 w-full max-w-7xl mx-auto px-6 sm:px-12 lg:px-16 py-20 md:py-32 space-y-7">
        ...
        <p class="text-neutral-300 text-sm sm:text-base leading-relaxed font-normal max-w-2xl">
        ...
        <div class="flex flex-wrap items-center gap-4 pt-2">
          <router-link to="/shop" class="px-8 py-4 bg-indigo-600 hover:bg-indigo-500 text-white font-semibold text-xs rounded-full transition-all shadow-lg shadow-indigo-600/25">
            Explore Catalog
          </router-link>
          <router-link to="/stores" class="px-8 py-4 bg-white/10 hover:bg-white/20 text-white font-semibold text-xs border border-white/20 transition-all rounded-full backdrop-blur-xs">
            Partner Stores
          </router-link>
        </div>
      </div>
    ...
    <div class="max-w-7xl mx-auto px-6 sm:px-12 lg:px-16 space-y-16">
      ...
      <section class="w-full space-y-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          <div
            ...
            :key="store.id"
            class="bg-white border border-neutral-200/80 p-6 hover:border-indigo-400/80 hover:shadow-xl transition-all duration-300 flex flex-col justify-between rounded-2xl group relative overflow-hidden"
          >
            <div class="space-y-4">
              <div class="flex items-center justify-between">
                <div class="w-12 h-12 bg-neutral-950 text-white flex items-center justify-center rounded-2xl shadow-md group-hover:bg-indigo-600 transition-colors">
                  <StoreIcon class="w-6 h-6" />
                </div>
                <span class="px-2.5 py-1 rounded-full bg-indigo-50 text-indigo-600 text-[11px] font-semibold">
                  Optical Boutique
                </span>
              </div>
              
              <div>
                <h3 class="font-bold text-lg text-neutral-900 tracking-tight group-hover:text-indigo-600 transition-colors">
                  {{ store.name }}
                </h3>
                <span class="text-xs font-mono text-indigo-600 block mt-0.5">/stores/{{ store.slug }}</span>
              </div>
            ...
            <div class="mt-6 pt-4 border-t border-neutral-100 flex items-center justify-between gap-3 text-xs">
              <div class="flex items-center gap-1.5 text-neutral-500 font-medium truncate min-w-0 flex-1">
                <MapPin class="w-3.5 h-3.5 text-indigo-600 shrink-0" />
                <span class="truncate">{{ formatAddress(store.address) }}</span>
              </div>

              <router-link
                :to="`/stores/${store.slug}`"
                class="text-indigo-600 hover:text-indigo-800 font-bold flex items-center gap-1 shrink-0 group-hover:translate-x-0.5 transition-transform"
              >
    ...
    <!-- Full Width Features Dark Banner -->
    <section class="w-full bg-neutral-950 text-white py-16 px-6 sm:px-12 lg:px-16 border-y border-neutral-800 shadow-xl">
      <div class="w-full max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-10">
...
import { Sparkles, ArrowRight, Store as StoreIcon, Eye, Truck, ShieldCheck, Layers, LayoutGrid, MapPin } from 'lucide-vue-next';
